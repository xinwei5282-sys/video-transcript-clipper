...
STALE_CLAIM_SEC = 180      # claimed 超过 3 分钟仍未完成 = 处理方掉线,退回 pending 供重试
...
# Whisper 在无人声(纯音乐/静音/水印)音频上,会吐训练数据里的固定幻觉短语。
# 这些短语真实抖音口播里几乎不会出现,命中即视为幻觉句剔除。
_HALLUCINATION_MARKERS = [
    "明镜与点点", "点点栏目", "请不吝点赞", "打赏支持",
    "字幕志愿者", "字幕由", "Amara.org", "感谢观看", "谢谢观看", "谢谢大家观看",
]


def strip_hallucination(text):
    """剔除含强幻觉标记的句子;若剔除后几乎不剩有效内容,判定为无清晰口播。"""
    if not text:
        return text
    import re
    kept = []
    for line in text.split("\n"):
        for seg in re.split(r"(?<=[。!?！？\n])", line):
            s = seg.strip()
            if s and not any(m in s for m in _HALLUCINATION_MARKERS):
                kept.append(seg)
    cleaned = "".join(kept).strip()
    residual = re.sub(r"[\s,，。!！?？;；、·\-—\.·…]+", "", cleaned)
    if len(residual) < 6:
        return "(本视频无清晰口播，未识别到有效文案)"
    return cleaned
...
def transcribe_one(wav_path, tmpdir, lang, tag="", model_path=None):
    ...
    cmd = [
        ...
        "-et", "10.0",
        "-mc", "64",
        # 不要加 -nt:no-timestamps 模式会抑制时间戳锚点 token,
        # 在部分音频上导致 whisper 解码跑飞/提前截断(只转出第一句 + 幻觉短语)。
        # -otxt 本身输出的就是无时间戳的纯文本,不需要 -nt。
        "-otxt", "-of", out_prefix, wav_path,
    ]
    if lang == "zh":
            ...
            with CLIP_LOCK:
                ...
                pending = [
                    ...
                    or (task["status"] == "claimed"
                        and now - task["updated_at"] > STALE_CLAIM_SEC)
                ]
